...
const getBookCopiesById = (request, response) => {
  if ((id = parseInt(request.params.id))) {
    return getBookCopiesByAttribute("bc.id", id, request, response);
  } else {
    return response.status(400).send({
...
const getBookCopiesByCallNumber = (request, response) => {
...
const getBookCopiesByShelfCode = (request, response) => {
  if ((shelfCode = parseInt(request.params.shelf_code))) {
    return getBookCopiesByAttribute(
      "bs.shelf_code",
      shelfCode,
      request,
  ...
  if (Object.keys(requestBody).length) {
    try {
      const result = await db.one(
        "INSERT INTO bookcopy(id, book_id, call_number, availability, format) VALUES(${id}, ${book_id}, ${call_number}, ${availability}, ${format}) RETURNING id",
        requestBody
      );
      return response.status(201).send({
        success: true,
